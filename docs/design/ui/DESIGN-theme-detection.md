...
### Plugin-Managed Variables

Variables generated by `_detectThemeColors()` for non-Material themes and consumed by the plugin:

| Variable | Source (Material) | Source (Non-Material) | Consumers |
|---|---|---|---|
| `--md-primary-fg-color` | Theme CSS | navbar `backgroundColor` | Controls gradient, focus header, mermaid header, mermaid expand btn |
| `--md-primary-fg-color--dark` | Theme CSS | `_darken(navBg, 0.15)` | Controls border, focus save hover |
| `--md-primary-bg-color` | Theme CSS | navbar `color` | Button text, label, focus header text, mermaid header text, mermaid expand btn text |
| `--md-footer-bg-color` | Theme CSS (or derived) | `_darken(navBg, 0.1)` | Controls gradient bottom |
| `--md-default-bg-color` | Theme CSS | body `backgroundColor` | Textarea bg, focus overlay bg, mermaid overlay bg |
...
| `--md-default-fg-color` | Theme CSS | body `color` | Textarea text, focus overlay text, mermaid overlay text |
| `--md-default-fg-color--light` | Theme CSS | `_alpha(bodyFg, 0.54)` | Focus toggle inactive text |
| `--md-default-fg-color--lighter` | Theme CSS | `_alpha(bodyFg, 0.32)` | Focus toggle border, checkbox border, mermaid block border |
...
| `--md-text-font-family` | Theme CSS | body `fontFamily` | Focus overlay font, mermaid overlay font |
| `--md-code-font-family` | Theme CSS (or derived) | body `fontFamily` | Textarea font |
| `--md-code-bg-color` | Theme CSS | `_darken/_lighten(bodyBg, 0.04/0.06)` | Mermaid preview bg, focus mode code block bg |
| `--md-code-fg-color` | Theme CSS | body `color` | Focus mode code block text |

### Material Mermaid Variables (Guard-Checked)

Variables defined by Material's mermaid JS integration, consumed exclusively by `_buildMaterialMermaidCSS()`. These are injected into rendered SVG `<style>` blocks only when the guard check confirms `--md-mermaid-node-bg-color` is defined on `document.body` (Material scopes mermaid variables to `[data-md-color-scheme]` on `<body>`, not `:root`). They have no `var()` fallbacks because the entire CSS block is omitted when the variables are absent — the SVG falls back to mermaid's default theme colors.

| Variable | Category | SVG Elements |
|---|---|---|
| `--md-mermaid-node-bg-color` | Core | Node shapes (rect, circle, ellipse, polygon, path), class/state groups, ER attribute boxes |
| `--md-mermaid-node-fg-color` | Core | Node strokes, class/state group strokes, dividers, ER attribute/entity strokes |
| `--md-mermaid-edge-color` | Core | Edge paths, flowchart links, arrowheads, markers, relations, transitions, state forks/joins |
| `--md-mermaid-label-fg-color` | Core | Labels, node labels, title text (flowchart, class, state, ER), cardinality, edge labels |
| `--md-mermaid-label-bg-color` | Core | Edge label backgrounds, class label boxes, state composites, ER entity/relationship boxes |
| `--md-mermaid-font-family` | Core | All label text, node labels, class/state/ER text |
| `--md-mermaid-sequence-actor-bg-color` | Sequence | Actor boxes |
| `--md-mermaid-sequence-actor-border-color` | Sequence | Actor box strokes |
| `--md-mermaid-sequence-actor-fg-color` | Sequence | Actor text |
| `--md-mermaid-sequence-actor-line-color` | Sequence | Lifelines |
| `--md-mermaid-sequence-actorman-bg-color` | Sequence | Stick figure fills |
| `--md-mermaid-sequence-actorman-line-color` | Sequence | Stick figure strokes |
| `--md-mermaid-sequence-message-line-color` | Sequence | Message lines, arrowheads |
| `--md-mermaid-sequence-message-fg-color` | Sequence | Message text |
| `--md-mermaid-sequence-note-bg-color` | Sequence | Note fills |
| `--md-mermaid-sequence-note-border-color` | Sequence | Note strokes |
| `--md-mermaid-sequence-note-fg-color` | Sequence | Note text |
| `--md-mermaid-sequence-loop-bg-color` | Sequence | Loop region fills |
| `--md-mermaid-sequence-loop-border-color` | Sequence | Loop region strokes |
| `--md-mermaid-sequence-loop-fg-color` | Sequence | Loop text |
| `--md-mermaid-sequence-label-bg-color` | Sequence | Label boxes |
| `--md-mermaid-sequence-label-fg-color` | Sequence | Label text |
| `--md-mermaid-sequence-number-fg-color` | Sequence | Sequence numbers |
| `--md-mermaid-sequence-number-bg-color` | Sequence | Sequence number backgrounds |
| `--md-mermaid-sequence-box-bg-color` | Sequence | Box region fills |
| `--md-mermaid-sequence-box-fg-color` | Sequence | Box region text |

Also uses `--md-default-fg-color--lightest`, `--md-default-fg-color--lighter`, and `--md-default-bg-color` from the plugin-managed table above (guaranteed present when Material is active).
...
- **Mermaid Mode**: `docs/design/mermaid/DESIGN-mermaid-mode.md` — SVG Theme Integration section documents how `_buildMaterialMermaidCSS` injects Material's CSS variable overrides into rendered mermaid SVGs, and explains the intentional `var()` fallback omission.
